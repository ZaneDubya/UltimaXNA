...
namespace UltimaXNA.UI
{
    class Window_Merchant : Window
    {
        ...
        private Container mContainerObject;
        ...
        public Window_Merchant(Entity nContainerObject, FormCollection nFormCollection)
            : base()
        {
            mContainerObject = (Container)nContainerObject;

            //Create a new form
            string iFormName = "frmMerchant:" + mContainerObject.Serial;
            _formCollection.Add(new Form(iFormName, "", mWindowSize, new Vector2(200, 200), Form.BorderStyle.None));
            _MyForm = _formCollection[iFormName];
        ...
        private void btnInv_OnPress(object obj, EventArgs e)
        {
            ...
            Item iItem = mContainerObject.Contents[iIndex];
            if (iItem != null)
            {
                // pick the item up!
        ...
        private void btnInv_OnRelease(object obj, EventArgs e)
        {
            ...
            // {
            //     int iIndex = Int32.Parse(((CustomButton)obj).Name.Substring(6)) + mScrollY * 2;
            //     UIHelper.DropItemIntoSlot(mContainerObject, iIndex);
            // }
        }
        ...
        public override void Update()
        {
            ...
            {
                mMaxScrollY = 100; // (int)(mContainerObject.Contents.LastSlotOccupied / 2) + 1 - 6;

                for (int i = 0; i < 12; i++)
                {
                    int iItemTypeID = 0;
                    Item iItem = mContainerObject.Contents[i + mScrollY * 2];
                    if (iItem != null)
                        iItemTypeID = iItem.ItemID;
                    string iBtnName = "btnInv" + i;
                    string iLblName = "lblItem" + i;
                    ((CustomButton)_MyForm[iBtnName]).Texture = UIHelper.ItemIcon(iItem);
                    if (iItemTypeID == 0)
                    {
