...
namespace UltimaXNA.SceneManagement
{
    public class LoggingInScene : BaseScene
    {
        ...
        {
            base.Intitialize();
            Gump g = UI.AddGump_Local(new UILegacy.ClientsideGumps.LoggingInGump(), 0, 0);
            ...
            if (SceneState == SceneState.Active)
            {
                switch (UltimaClient.Status)
                {
                    ...
                    case UltimaClientStatus.LoginServer_WaitingForLogin:
                        ...
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 9;
                        UltimaClient.Login();
                        break;
                    ...
                    case UltimaClientStatus.Error_CannotConnectToServer:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 2;
                        // could not connect to server.
                        break;
                    case UltimaClientStatus.Error_InvalidUsernamePassword:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 3;
                        break;
                    case UltimaClientStatus.Error_InUse:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 4;
                        break;
                    case UltimaClientStatus.Error_Blocked:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 5;
                        break;
                    case UltimaClientStatus.Error_BadPassword:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 6;
                        break;
                    case UltimaClientStatus.Error_Idle:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 7;
                        break;
                    case UltimaClientStatus.Error_BadCommunication:
                        UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).ActivePage = 8;
                        break;
                    default:
        ...
        {
            UI.GetGump<UILegacy.ClientsideGumps.LoggingInGump>(0).Dispose();
            base.Dispose();
        }
